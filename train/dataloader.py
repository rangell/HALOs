...
def get_tiny_unsafe(split: str) -> Dataset:
    """
    ...
    with open("tiny_unsafe_preference_data.pkl", "rb") as f:
        dataset = pickle.load(f)

    data = Dataset("tiny_unsafe")
...
class UnsafeUnaryDataLoader(UnpairedPreferenceDataLoader):
    def get_flat_data(self, prompts):
        """
        Return a flat list of examples given a list of prompts that index self.full_data.
        ...
        for prompt in prompts:
            example = self.full_data[prompt]
            flat_data.append((example, example.generations[0], "chosen" if example.desirable[0] else "rejected"))
        return flat_data
